...
export const POST: APIRoute = async ({ request }) => {
  ...
  const order = await env.DB.prepare(
    `SELECT orders.id, orders.status, orders.total_cents, orders.product_name, orders.size_name, orders.wood,
     ...
     WHERE orders.id = ? AND lower(customers.email) = ? AND orders.status IN ('draft', 'photos_received', 'awaiting_payment')
       AND (SELECT COUNT(*) FROM uploads WHERE uploads.order_id = orders.id) >= 3`,
  ).bind(orderId, email).first<{ id: string; status: string; total_cents: number; product_name: string; size_name: string | null; wood: string | null; email: string; pet_name: string; stripe_checkout_session_id: string | null; stripe_checkout_attempt: number }>();
